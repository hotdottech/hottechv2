...
type SponsorBlockModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: SponsorBlockData) => void;
  initialData: SponsorBlockData;
};
...
export function SponsorBlockModal({
  ...
  const [data, setData] = useState<SponsorBlockData>(initialData);

  useEffect(() => {
    if (isOpen) setData(initialData);
  }, [isOpen, initialData]);

  const update = useCallback((partial: Partial<SponsorBlockData>) => {
  ...
  const updateItem = useCallback((index: number, partial: Partial<SponsorItem>) => {
    setData((prev) => {
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <div>
              <label className="block font-sans text-xs text-gray-500">Title Tag</label>
              <select
                value={data.titleTag}
                onChange={(e) => update({ titleTag: e.target.value as SponsorBlockData["titleTag"] })}
            ...
            <div>
              <label className="block font-sans text-xs text-gray-500">Title Color</label>
              <select
                value={data.titleColor}
                onChange={(e) => update({ titleColor: e.target.value as SponsorBlockData["titleColor"] })}
                className="mt-1 w-full rounded-md border border-white/10 bg-hot-black px-3 py-2 font-sans text-sm text-hot-white focus:border-hot-white/30 focus:outline-none"
              >
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <div>
              <label className="block font-sans text-xs text-gray-500">Columns</label>
              <select
                value={data.columns}
                onChange={(e) => update({ columns: e.target.value as SponsorBlockData["columns"] })}
                className="mt-1 w-full rounded-md border border-white/10 bg-hot-black px-3 py-2 font-sans text-sm text-hot-white focus:border-hot-white/30 focus:outline-none"
              >
            ...
            <div>
              ...
              <select
                value={data.size}
                onChange={(e) => update({ size: e.target.value as SponsorBlockData["size"] })}
                className="mt-1 w-full rounded-md border border-white/10 bg-hot-black px-3 py-2 font-sans text-sm text-hot-white focus:border-hot-white/30 focus:outline-none"
              >
          ...
          <div>
            <div className="flex items-center justify-between">
              <label className="block font-sans text-xs text-gray-500">Sponsors / Partners</label>
              <button
                type="button"
                onClick={addItem}
            ...
            <div className="mt-3 space-y-4">
              {data.items.map((item, index) => (
                <div key={item.id} className="rounded-md border border-white/10 bg-hot-black/30 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="font-sans text-xs text-gray-500">Logo {index + 1}</span>
                    <button
                      type="button"
                  ...
                  <div className="space-y-3">
                    ...
                    />
                    <div>
                      <label className="block font-sans text-xs text-gray-500">Link URL</label>
                      <input
                        type="url"
                        value={item.linkUrl ?? ""}
                    ...
                    <div>
                      <label className="block font-sans text-xs text-gray-500">Label</label>
                      <input
                        type="text"
                        value={item.partnerLabel ?? ""}
                        onChange={(e) => updateItem(index, { partnerLabel: e.target.value })}
                        placeholder="e.g. Official TV Partner"
                        className="mt-1 w-full rounded-md border border-white/10 bg-hot-black px-3 py-2 font-sans text-sm text-hot-white placeholder-gray-500 focus:border-hot-white/30 focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block font-sans text-xs text-gray-500">Alt text (optional)</label>
                      <input
                        type="text"
                        value={item.altText ?? ""}
            ...
            {data.items.length === 0 && (
              <p className="mt-2 font-sans text-sm text-gray-500">No sponsors yet. Click Add to add a logo.</p>
            )}
          </div>
        </div>
